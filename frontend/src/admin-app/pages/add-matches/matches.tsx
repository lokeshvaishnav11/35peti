...
const MatchesPage = () => {
  ...
  // ================= SELECT SINGLE MATCH =================
  const selectMatch = (e: ChangeEvent<HTMLInputElement>, index: number) => {
    const items = [...matches]
    items[index].active = e.target.checked
    setMatches(items)

    // update select all checkbox
    const allChecked = items.every((item) => item.active)
  ...
  const handleMatch = (e: FormEvent<HTMLFormElement>) => {
    ...
    seriesService
      .saveMatch(selectedMatches)
      .then(() => toast.success('Match Saved Successfully'))
        ...
        <div className="col-md-12 main-container">
          <form onSubmit={handleMatch}>
            ...
            <table className="table table-bordered">
              ...
              <tbody>
                {matches.map((match: IMatch, index: number) => {
                  if (match?.series?.id == '1') return null
                    ...
                    <tr key={index}>
                      ...
                      <td>
                        {new Intl.DateTimeFormat('en-US', {
                          ...
                          minute: '2-digit',
                          second: '2-digit',
                        }).format(new Date(match.matchDateTime))}
                      </td>
                      <td>
                        <input
                          type="checkbox"
