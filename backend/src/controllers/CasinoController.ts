...
export class CasinoController extends ApiController {
  ...
  try {
    ...
    let allowCasinoIds: number[] = []

    // 2️⃣ user ka AllowCasino
    if (Array.isArray(user.AllowCasino) && user.AllowCasino.length > 0) {
      allowCasinoIds = user.AllowCasino.map(Number)
    }

    // 3️⃣ parent ka AllowCasino
      ...
      if (
        parentUser &&
        Array.isArray(parentUser.AllowCasino) &&
        parentUser.AllowCasino.length > 0
      ) {
        allowCasinoIds = parentUser.AllowCasino.map(Number)
      }
    }

    // 4️⃣ casino filter
    let casinoFilter: any = {
      status: { $in: [1] },
    }

    if (allowCasinoIds.length > 0) {
      casinoFilter.match_id = { $in: allowCasinoIds }
    }

    // 5️⃣ fetch casinos
    const games = await Casino.find(casinoFilter)
      .sort({ order: 1 })
      .select({
        match_id: 1,
        slug: 1,
    ...
    return this.success(res, games, 'Data Found')
